...
TEMPLATE = dedent("""\
    ...
    config IRONSIDE_SE_VERSION_LATEST_STRING
    \tstring
    {platform_latest_strings}
    \thelp
    \t  The latest supported verion of IronSide SE for the current SoC, in string format.
...
def main() -> None:
    ...
    for soc, soc_output in output_by_soc.items():
        ...
        prefix = version.lower()
        platform_latest_string_lines.append(
            PLATFORM_LATEST_STRING_TEMPLATE.format(
                prefix=prefix, config_depends=MDK_DEFINE_TO_SOC[soc]
            )
        )
    ...
    platform_latest_strings = "\n".join(sorted(platform_latest_string_lines))
    ...
    file_content = TEMPLATE.format(
        ...
        platform_latest_strings=platform_latest_strings,
    )
