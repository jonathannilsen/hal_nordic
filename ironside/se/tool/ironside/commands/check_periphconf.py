...
class InputFileAction(argparse.Action):
    """Custom Action fo handling PERIPHCONF file inputs."""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if not file_name.exists():
            raise argparse.ArgumentError(self, f"{file_name} does not exist")
            ...
            case OffsetFormat.MCUBOOT_TLV:
                ...
                try:
                    ...
                    raise argparse.ArgumentError(
                        self, f"failed to read TLV data from '{desc_bin}'"
                    ) from err
...
def cmd_handler(args: argparse.Namespace) -> None:
    ...
    table_str = render_periphconf_table(entries_to_print, sources, style=args.style)
